...
:: Start the local API server used by the browser question generator.
netstat -ano | findstr 127.0.0.1:8000 > nul
if %errorlevel% neq 0 (
    echo [1/2] Starting local API server on port 8000...
    start /min "PBL API Server" python server.py
    timeout /t 1 > nul
) else (
    echo [1/2] API server is already running.
)
